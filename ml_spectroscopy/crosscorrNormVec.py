...
def crosscorrRV_vec(w, f, tw, tf, rvmin, rvmax, drv, mode="doppler", normalized=True, skipedge=10, edgeTapering=None):
  """
    ...
    f : 2D array: dim 1 = number of spaxels to filter, dim2=number of wavelengths (same as wavelength length)
...
# =============================================================================
  ...
  cc = np.zeros((int(f.shape[0]),len(drvs)))
  ...
  if mode == "lin":
      if normalized==True:
          ...
          # Shift the template linearly
              ...
              cc[:,i] = np.sum(f * fi(w),axis=1)/np.sqrt(np.sum(f*f, axis=1)*np.sum(fi(w)*fi(w)))
      else:
          for i, rv in enumerate(drvs):
              fi = sci.interp1d(tw+meanWl*(rv/c), tf)
