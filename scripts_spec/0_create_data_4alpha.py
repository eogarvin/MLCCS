...
def spectrum_dataset_4alpha_norep(data0, planet_signals0, mol, balancing=50, seed=100):
    import random
    random.seed(seed)
    ...
    for ind in range(0, int(length_levels)):
        ...
        data_signal = data_signal.append(tmp_signals)
        data_noise = data_noise.append(tmp_noise)
...
for m in mol:
    for i in range(0, len(datasets)):
        for b in bal:
            ...
            noise, signal = spectrum_dataset_4alpha_norep(data_init, Planet_Signals, mol=m, balancing=b, seed = s)

            noise.to_pickle(data_path + "Noise4alpha_bal" + str(b) + "_mol" + m + "_simple.pkl")
            signal.to_pickle(data_path + "Signal4alpha_bal" + str(b) + "_mol" + m + "_simple.pkl")
