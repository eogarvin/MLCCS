...
mol2 = 'CO'
mols = [mol1, mol2]
# Make a new dataset with 1/3 CO 1/3 Water, 1/3 noise.
# Cross correlate it both with water and co.
# Add a S/N, PCT, CNN which evaluate each and one that evaluates both.
...
for alpha in alpha_vec:
    ...
    data1 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol1 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp1200.0_sg4.1_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')
    data2 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol2 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp1400.0_sg4.1_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')
    data3 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol1 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp1600.0_sg4.1_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')
    data4 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol2 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp2000.0_sg4.1_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')
    data5 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol1 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp2500.0_sg4.1_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')
    data6 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol2 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp1200.0_sg2.9_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')
    data7 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol1 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp1400.0_sg3.5_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')
    data8 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol2 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp1600.0_sg4.5_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')
    data9 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol1 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp2000.0_sg5.3_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')
    data10 = pd.read_pickle(data_path + 'data_4ml/v'+str(version)+'_ccf_4ml_trim_robustness_simple/' + mol2 + '_' + data_name + '_scale' + str(
        alpha) + '_bal' + str(bal) + '_temp2500.0_sg5.3_ccf_4ml_trim_norepetition_v'+str(version)+'_simple.pkl')

    datas = {0: data1, 1: data2, 2: data3, 3: data4, 4: data5, 5: data6, 6: data7, 7: data8, 8: data9, 9: data10}
    methods = ['SNR', 'SNR_auto', 'PCT', 'DNN', 'CNN1', 'CNN2', 'ENET', 'RID', 'LAS', 'RF', 'XGB', 'ENET2']
    len_index = len(data1[0].index.levels[0])


    X_train = {0: None, 1: None, 2: None, 3: None, 4: None, 5: None, 6: None, 7: None, 8: None, 9: None}
    X_valid = {0: None, 1: None, 2: None, 3: None, 4: None, 5: None, 6: None, 7: None, 8: None, 9: None}
    X_test = {0: None, 1: None, 2: None, 3: None, 4: None, 5: None, 6: None, 7: None, 8: None, 9: None}

    y_train = {0: None, 1: None, 2: None, 3: None, 4: None, 5: None, 6: None, 7: None, 8: None, 9: None}
    y_valid = {0: None, 1: None, 2: None, 3: None, 4: None, 5: None, 6: None, 7: None, 8: None, 9: None}
    y_test = {0: None, 1: None, 2: None, 3: None, 4: None, 5: None, 6: None, 7: None, 8: None, 9: None}


    for d in datas:

        data_train = datas[d].drop([(str(datas[d].index.levels[0][j]),)], axis=0).drop([(str(datas[d].index.levels[0][i]),)], axis=0)
        ...
        data_train.insert(data_train.shape[1], "planet", 0)
        X_train[d] = data_train.drop(['tempP', 'loggP', 'H2O', 'CO', 'CH4', 'NH3', 'subclass', 'planet'], axis=1)
        #cols_train = data_train.columns[data_train.columns.isin(mols)]
        data_train.loc[data_train[(data_train[mols] == 1).any(axis=1)].index, 'planet'] = 1
        Y_train = data_train['planet']
        # y_train[d] = Y_train

        data_valid.insert(data_valid.shape[1], "planet", 0)
        X_valid[d] = data_valid.drop(['tempP', 'loggP', 'H2O', 'CO', 'CH4', 'NH3', 'subclass', 'planet'], axis=1)
        #cols_valid = data_valid.columns[data_valid.columns.isin(mols)]
        data_valid.loc[data_valid[(data_valid[mols] == 1).any(axis=1)].index, 'planet'] = 1
        Y_valid = data_valid['planet']
        # y_valid[d] = Y_valid

        data_test.insert(data_test.shape[1], 'planet', 0)
        X_test[d] = data_test.drop(['tempP', 'loggP', 'H2O', 'CO', 'CH4', 'NH3', 'subclass', 'planet'], axis=1)
        #cols_test = data_test.columns[data_test.columns.isin(mols)]
        data_test.loc[data_test[(data_test[mols] == 1).any(axis=1)].index, 'planet'] = 1
    ...
    x_train = np.stack((np.array((X_train[0])), np.array((X_train[1])), np.array((X_train[2])), np.array((X_train[3])),
                        ...
                        np.array((X_train[8])), np.array((X_train[9]))), axis=-1)
    x_valid = np.stack((np.array((X_valid[0])), np.array((X_valid[1])), np.array((X_valid[2])), np.array((X_valid[3])),
                        ...
                        np.array((X_valid[8])), np.array((X_valid[9]))), axis=-1)
    x_test = np.stack((np.array((X_test[0])), np.array((X_test[1])), np.array((X_test[2])), np.array((X_test[3])),
                       ...
                       np.array((X_test[8])), np.array((X_test[9]))), axis=-1)



    # Data stack all
    x_train = np.stack((np.array((X_train[0])), np.array((X_train[1])), np.array((X_train[2])), np.array((X_train[3])),np.array((X_train[4])), np.array((X_train[5])), np.array((X_train[6])), np.array((X_train[7])),np.array((X_train[8])), np.array((X_train[9]))), axis=-1)
    x_valid = np.stack((np.array((X_valid[0])), np.array((X_valid[1])), np.array((X_valid[2])), np.array((X_valid[3])),np.array((X_valid[4])), np.array((X_valid[5])), np.array((X_valid[6])), np.array((X_valid[7])),np.array((X_valid[8])), np.array((X_valid[9]))), axis=-1)
    x_test = np.stack((np.array((X_test[0])), np.array((X_test[1])), np.array((X_test[2])), np.array((X_test[3])),np.array((X_test[4])), np.array((X_test[5])), np.array((X_test[6])), np.array((X_test[7])),np.array((X_test[8])), np.array((X_test[9]))), axis=-1)
    ...
    results = {'results': res_opt_method, 'confusion matrix': CM_opt, 'hyperparameters': hyperparam_optim,
               'y_test': list(y_test)}
    a_file = open(results_path + "export_CV/from_GPU_byfold/results_" + "_".join(mols) +"_" +str(data_name)+"_data_"+str(x)+"_alpha_"+str(alpha)+"_CV_testfold" + str(j) + "_multi.pkl", "wb")
    pickle.dump(results, a_file)
    a_file.close()

    # export optimizaiton results
    a_file = open(results_path + "export_CV/from_GPU_byfold/GA_results_" + "_".join(mols) +"_" + str(data_name)+"_data_"+str(x)+"_alpha_"+str(alpha)+"_CV_testfold" + str(j) + "_multi.pkl", "wb")
