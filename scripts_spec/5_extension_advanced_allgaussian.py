...
data_name = 'GQlupb'
planet = 'GQlupB'
alpha_vals= [5,9,16] # [2,5,8,11,22,32,43,55]
#alpha=2
bal=50
v=6
frame='simple'
...
dir_path = results_path + "export_CV/from_GPU_byfold/Res_tmp_alphas_advanced_all_gaussian_aperture/"  # Depending on the way we pick the files, can't we directly point to the right directory from the start?
#Directory to store results
visualisation_path = subdir + "80_visualisation/Realistic_fakeplanets/Vis_04_090523_alphas_advanced_realisticfake_all_gaussian_aperture/"
csv_path = subdir + "80_visualisation/Realistic_fakeplanets/Vis_04_090523_alphas_advanced_realisticfake_gaussian_all_aperture/"
...
for alpha in alpha_vals:
    ...
    tp_path = '/home/ipa/quanz/user_accounts/egarvin/Thesis/70_results/export_CV/from_GPU_byfold/Res_tmp_alphas_advanced_all_gaussian_aperture/'
    keys = folds
    ...
    for j in folds: # + 1):
        ...
        for m in methods_ls:
            ...
            if m in ['ENET', 'LAS', 'RID', 'ENET2', 'XGB']:
                ...
                Y_pred = np.array(prob_Y > 0.6) * 1
                #prob_Y = ls_results_realistic_fake[j]
            ...
            # dt_true = pd.read_pickle(data_path + "csv_inputs/CCF_realistic_fakeplanets/noise_and_planets_spectra/injection_labels_set.pkl")
            # ind = pd.MultiIndex.from_tuples(dt_true.index, names=['planet', 'spaxel'])
            # series_true = pd.DataFrame(dt_true, index=ind)
            # Y_true=np.array(series_true.loc[(str(series_true.index.levels[0][j]), slice(None))])
            # reconstruct_true = np.tile(Y_true, 2).reshape(2, len(Y_true)).T
            ...
            dt_true = data1['H2O']
            Y_true=np.array(dt_true.loc[(str(dt_true.index.levels[0][j]), slice(None))])
            reconstruct_true = np.tile(Y_true, 2).reshape(2, len(Y_true)).T
            ...
            img_insertion = image_reconstruct(reconstruct_true, Planet_vec_shape[0], Planet_vec_shape[1],
                                           Planet_position_nan)



            # prediction
            plt.imshow(img_insertion[0, :, :])
            plt.title('test reconstruction true value')
            plt.show()
            plt.close()
            ...
            plt.savefig(
                visualisation_path + 'img_realisticfake_' + planetlist[j] + '_method_' + str(m) + '_alpha_' + str(
                    alpha) + '_Prediction_areas.pdf') # bbox_inches='tight')
            plt.close()
