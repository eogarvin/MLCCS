...
######
# TODO: drv can be varied now, but only with drv = [2,5,10 and multiples of 10). We adapted SNR etc but it only works with an RV 0 in the middle (relies on this for calculations of the limits of CCF signal and also for the RV peak.
######
...
print(time_it)
t0 = open(data_path+'/time_it.txt', 'w')
...
results = test_onCCF_rv0_SNR_drv(tmp_data_frame, drv=1, snr=3.0)
...
for tmp_alpha in tqdm(alpha_values):
    ...
    tmp_results = test_onCCF_rv0_SNR_drv(tmp_data_frame, drv=1, snr=3.0)
