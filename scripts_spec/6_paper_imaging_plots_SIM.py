...
dir_path = results_path + "export_CV/from_GPU_byfold/Res_SIM_planets_091223/"  # Depending on the way we pick the files, can't we directly point to the right directory from the start?
# Directory to store results
visualisation_path = subdir + "80_visualisation/Realistic_fakeplanets/Vis_07_091223_SIM/"
csv_path = subdir + "80_visualisation/Realistic_fakeplanets/Vis_07_191223_SIM/"

k2 = [0, 1, 2]
#alpha_set = ["alphanone", "alphaover3", "alphaover6", "alphamin"]
alpha_set = ["alphaover3","alphaover6", "alphamin"]
...
for a in range(len(alpha_set)):
    ...
    tp_path = '/home/ipa/quanz/user_accounts/egarvin/Thesis/70_results/export_CV/from_GPU_byfold/Res_SIM_planets_091223/'
    # keys = folds
    # ls_results_realistic_fake = {key: None for key in keys}
    # for i in folds:
    with open(tp_path + 'results_SIM_data_alpha_'+alpha_set[a]+'_CV_testfold.pkl', "rb") as f:
        ls_results_realistic_fake = pickle.load(f)  # i is the validation number but the proper set is at i+1



    data00 = pd.read_pickle(
        data_path + 'csv_inputs/intro_plot_dataset/PZ_Tel_signals_alphanone_temp2800_logg_4.1_CO1.pkl')
    data0 = data00.loc[data00.index.levels[0][k2]]


    data01 = pd.read_pickle(
        data_path + 'csv_inputs/intro_plot_dataset/PZ_Tel_signals_' + str(alpha_set[a]) + '_temp2800_logg_4.1_CO1.pkl')
    ...
    meta_data = pd.read_pickle(data_path + "csv_inputs/intro_plot_dataset/PZ_Tel_signals_alphanone_CO1_meta.pkl")
    # For each fold:
    # get the results
    ...
    # else:
    # i = j - 1
    data_test_original_sim  = data0.loc[data0.index.levels[0][k2]].drop("H2O", axis=1)

    data_test = data1.loc[data1.index.levels[0][k2]].drop("H2O", axis=1)
    ...
    fig2.suptitle(r"Grid of PZ Tel B simulations inserted in real SINFONI noise and resulting H$_2$O maps ("+str(int(a+1))+")", fontsize=20, fontweight="bold")
    ...
    for i in range(3):
        ...
        for m in methods_ls:
            ...
            #
            # # CCF
            # plt.imshow(img_ccf_original_sim[0, :, :], cmap='viridis')
            # plt.title('Good Conditions', fontsize=18)
            # clb = plt.colorbar()
            ...
            #
            # # CCF
            # plt.imshow(img_ccf[0, :, :], cmap='viridis')
            ...
            # #        alpha) + '_CCF.pdf', bbox_inches='tight')
            #
            # plt.imshow(image_scores[m][1, :, :], cmap='viridis')
        ...
        fig_title = fig1.suptitle("PZ Tel B simulation inserted in real SINFONI noise and recovery of "+r"H$_2$O map", fontsize=20, fontweight="bold")

            # CCF
        img0 = ax1[0].imshow(img_ccf_original_sim[0, :, :], extent=extent0, cmap='viridis')
        ...
        ax1[0].label_outer()
                ...
                # CCF
        img1= ax1[1].imshow(img_ccf[0, :, :], extent=extent0, cmap='viridis')
        ...
        for s in range(2):
                # score
            img2 = ax1[2+s].imshow(image_scores[ms[s]][1, :, :], extent=extent0,  cmap='viridis')
        ...
        fig1.savefig(visualisation_path + 'SIM_alpha_' + str(alpha_set[a]) + '_Scores_'+str(fold_name[i])+'.pdf', bbox_inches='tight', dpi=600)
        fig1.show()
                #plt.savefig(
                #    visualisation_path + 'img_realisticfake_' + planetlist[j] + '_method_' + str(m) + '_alpha_' + str(
                #        alpha) + '_Scores.pdf', bbox_inches='tight')
            ...
            # CCF
        img0 = ax2[i,0].imshow(img_ccf_original_sim[0, :, :], extent=extent0, cmap='viridis')
        if i == 0:
        ...
        ax2[i,0].label_outer()
                ...
                # CCF
        img1= ax2[i,1].imshow(img_ccf[0, :, :], extent=extent0, cmap='viridis')
        if i == 0:
        ...
        for s in range(2):
                # score
            img2 = ax2[i,2+s].imshow(image_scores[mt[s]][1, :, :], extent=extent0,  cmap='viridis')
            if i == 0:
    ...
    fig2.savefig(visualisation_path + 'SIM_alpha_' + str(alpha_set[a]) + '_Scores_allfolds.pdf', bbox_inches='tight', dpi=400)
    fig2.show()
    plt.close()
