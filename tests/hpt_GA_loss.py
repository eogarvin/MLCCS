...
def genetic_algorithm(objective, bounds, n_bits, n_iter, n_pop, r_cross, r_mut, X_train, y_train, X_valid, y_valid):
	...
	best, accuracy_scores = pop[0], itemgetter('loss_valid_test', 'loss_train')(objective(decode(bounds, n_bits, pop[0]), X_train, y_train, X_valid, y_valid))#['accuracy_valid_test']
	#pop[0], itemgetter('accuracy_valid_test', 'accuracy_train')(objective(decode(bounds, n_bits, pop[0]), X_train, y_train, X_valid, y_valid))#['accuracy_valid_test']
	...
	for _ in range(n_iter):
		...
		accuracies_scores = [itemgetter('loss_valid_test', 'loss_train')(objective(d, X_train, y_train, X_valid, y_valid)) for d in decoded] # objective(d, X_train, y_train, X_valid, y_valid)['accuracy_valid_test']
		#[itemgetter('accuracy_valid_test', 'accuracy_train')(objective(d, X_train, y_train, X_valid, y_valid)) for d in decoded] # objective(d, X_train, y_train, X_valid, y_valid)['accuracy_valid_test']
		temp_real_accuracy_track0 = [itemgetter('accuracy_valid_test')(objective(d, X_train, y_train, X_valid, y_valid)) for d in decoded]
		#or
		#it = 0
		#accuracies_scores=[]
		#for d in decoded:
		...
		for i in range(n_pop):
			if scores[i] < best_eval:
				...
				temp_real_accuracy_track = temp_real_accuracy_track0[accuracies_valid_test0.index(best_eval)]
		...
		real_accuracy_track.append(temp_real_accuracy_track)
	...
	return [best, best_eval, real_accuracy_track, best_accuracies_valid, track_generation, track_hyperparams]
