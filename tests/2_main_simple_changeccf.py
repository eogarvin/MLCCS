...
# To adapt
mol = 'H2O'
version = 12 #6 base tests, 7 CO tests , 8 tests withgaussian noise, 9, multi tests # 10 lower rvs 1200 and 5 # 11lower rvs 10 #12 higher rvs 3000
...
for alpha in alpha_vec:
    ...
    for d in datas:
        ...
        X_train[d] = data_train.drop(['tempP', 'loggP', 'H2O', 'CO', 'CH4', 'NH3', 'subclass'], axis=1) #.drop(np.arange(-2000, -1200), axis = 1).drop(np.arange(1201, 2000), axis = 1)
        Y_train = data_train[mol]
        # y_train[d] = Y_train

        X_valid[d] = data_valid.drop(['tempP', 'loggP', 'H2O', 'CO', 'CH4', 'NH3', 'subclass'], axis=1) #.drop(np.arange(-2000, -1200), axis = 1).drop(np.arange(1201, 2000), axis = 1)
        Y_valid = data_valid[mol]
        # y_valid[d] = Y_valid

        X_test[d] = data_test.drop(['tempP', 'loggP', 'H2O', 'CO', 'CH4', 'NH3', 'subclass'], axis=1) #.drop(np.arange(-2000, -1200), axis = 1).drop(np.arange(1201, 2000), axis = 1)
    ...
    SNR_best = SNR_grid_search_drv(hyperparams_SNR, test_onCCF_rv0_SNR_drv, drv, X_train[x], y_train[:, 1])
    # end time
    end_o_SNR = time.time()
    # results:
    start_m_SNR = time.time()
    res_opt_method['SNR'] = test_onCCF_rv0_SNR_drv(X_test[x], drv, SNR_best['optimal_hyperparam'])
    end_m_SNR = time.time()
    ...
    results = {'results': res_opt_method, 'confusion matrix': CM_opt, 'hyperparameters': hyperparam_optim,
               'y_test': list(y_test)}
    a_file = open(results_path + "export_CV/from_GPU_byfold/results_" + mol +"_" +str(data_name)+"_data_"+str(x)+"_alpha_"+str(alpha)+"_CV_testfold" + str(j) + "_ccf3000.pkl", "wb")
    pickle.dump(results, a_file)
    a_file.close()

    # export optimization results
    a_file = open(results_path + "export_CV/from_GPU_byfold/GA_results_" + mol +"_" + str(data_name)+"_data_"+str(x)+"_alpha_"+str(alpha)+"_CV_testfold" + str(j) + "_ccf3000.pkl", "wb")
